 ...
 */
@injectable()
export default class MoveUpDownMovement implements IMovementBehavior {
  ...
  public init (): void {
    ...
    this._speed = new p5.Vector(0, random(SPEED_DEFAULT_MIN, SPEED_DEFAULT_MAX));
  };
  ...
  public update (): void {
    ...
    const { height } = this.p5Instance.sketch;

    const scaleFactor = (this.p5Instance.sketch.deltaTime ?? 1000) / 1000;
    this.position.add(0, this._speed.y * scaleFactor);
    if (this.position.y > height * 0.9 || this.position.y < height * 0.1) {
      this._speed.y *= -1;
    }
  };
...
export const MoveUpDownMovementName = Symbol.for('MoveUpDownMovementName');
